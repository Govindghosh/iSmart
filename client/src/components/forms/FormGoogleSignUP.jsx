...
const FormGoogleSignUP = () => {
  ...
  const handleGoogleSignup = () => {
    ...
    if (!baseURL) {
      ...
      alert("Google signup failed: Missing API base URL");
      return;
    }
      ...
      <div className="relative z-10 w-full max-w-4xl bg-white bg-opacity-90 rounded-3xl shadow-xl p-8 md:p-12 overflow-hidden">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" encType="multipart/form-data" noValidate>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 text-white font-semibold bg-gradient-to-r from-blue-500 to-purple-500 rounded-xl shadow-lg hover:opacity-90 transition disabled:opacity-60"
          >
